...
class AttConvLSTM(nn.Module):
    ...
    def beam_forward(self, feats, first_vertex, poly=None,
        ...
        v_prev2 = torch.zeros(fp_beam_batch_size, 1, self.grid_size, self.grid_size, device=self.device)
        v_prev1 = torch.zeros(fp_beam_batch_size, 1, self.grid_size, self.grid_size, device=self.device)
        v_first = torch.zeros(fp_beam_batch_size, 1, self.grid_size, self.grid_size, device=self.device)

        # Tokens table stores beam history
        tokens_table = torch.ones(fp_beam_batch_size, beam_size, self.time_steps, device=self.device)
        ...
        alive = torch.ones(fp_beam_batch_size, beam_size, 1, device=self.device)
        ...
        for t in range(1, self.time_steps):
            if t == 1:
                ...
                # Setup tensors to be reused for t>1
                v_prev2 = torch.zeros(full_beam_batch_size, 1, self.grid_size, self.grid_size, device=self.device)
                v_prev1 = torch.zeros(full_beam_batch_size, 1, self.grid_size, self.grid_size, device=self.device)
                v_first = torch.zeros(full_beam_batch_size, 1, self.grid_size, self.grid_size, device=self.device)
